...
png("Img/corrSpYr.png", width = pltWidth, height = pltHeight)  
ggplot(data = subset(corrres, illness == "Influenza" & 
  ...
  geom_line(aes(x = year, y = corr, color = var, group = var), size = 1.2) +
...
png("Img/corrSpIll.png", width = pltWidth, height = pltHeight)  
...
png("Img/corrLag.png", width = pltWidth, height = pltHeight)  
ggplot(data = subset(corrres, type =="Cross-correlation"  & year == -1
                     & var != "Positive test percent")) +
  mytheme + labs(x = "", y = "Lag",
                 title = "Number of weeks resipatory illnesses lag GFT in cross-correlation") +
